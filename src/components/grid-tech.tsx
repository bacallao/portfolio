...
const IconsFeature = () => {
    ...
    const techLogos = [
        ...
        { name: "Google", slug: "googlegemini" },
        { name: "Mistral", slug: "mistralai" },
        { name: "Ollama", slug: "ollama" },
    ];

    // Helper to get SVG icon CDN url for color mode
    function getLogoUrl(slug: string, mode: "white" | "colored") {
        if (mode === "white") {
            ...
            {techLogos.map((logo) => (
                ...
                >
                    <div className="relative w-8 h-8 flex items-center justify-center">
                        {/* White logo visible by default, fades out on hover */}
                        <img
                            src={getLogoUrl(logo.slug, "white")}
                            alt={logo.name}
                            className="w-7 h-7 object-contain opacity-60 group-hover:opacity-0 absolute transition-opacity duration-300"
                        />
                        {/* Colored logo visible on hover */}
                        <img
                            src={getLogoUrl(logo.slug, "colored")}
                            alt={logo.name}
                            className="w-7 h-7 object-contain opacity-0 group-hover:opacity-100 relative transition-opacity duration-300"
                        />
                    </div>
                        ...
                        {item.icons && !item.feature && (
                            <div className="mt-auto pt-4 flex items-center flex-wrap gap-4 border-t border-neutral-200/70 dark:border-neutral-800/70">
                                {[
                                    ...
                                    { slug: "googlegemini", name: "Google" },
                                    { slug: "mistralai", name: "Mistral" },
                                    { slug: "ollama", name: "Ollama" }
                                ].map((logo) => (
                                    <div key={logo.slug} className="relative w-5 h-5 group/icon">
                                        <img 
                                            src={`https://cdn.simpleicons.org/${logo.slug}/white`} 
                                            alt={logo.name} 
                                            className="w-5 h-5 opacity-60 group-hover/icon:opacity-0 absolute transition-opacity duration-300" 
                                        />
                                        <img 
                                            src={`https://cdn.simpleicons.org/${logo.slug}`} 
                                            alt={logo.name} 
                                            className="w-5 h-5 opacity-0 group-hover/icon:opacity-100 relative transition-opacity duration-300" 
                                        />
                                    </div>
                                ))}
                            </div>
